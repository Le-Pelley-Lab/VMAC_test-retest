...
if exptPhase == 0
    ...
    DATA.practrialInfoSpatial = zeros(pracTrials, 9);
    ...
    DATA.expttrialInfoSpatial = zeros(exptTrials, 15);
...
%             rtStr = ['RT = ', num2str(roundRT), ' milliseconds'];
    ...
    if exptPhase == 0
        DATA.practrialInfoSpatial(trial,:) = [str2double(exptSession), trial, targetLoc, targetType, distractLoc, distractType, timeout, correct, rt];
    else
        DATA.expttrialInfoSpatial(trial,:) = [str2double(exptSession), block, trial, trialCounter, trials_since_break, targetLoc, targetType, distractLoc, distractType, timeout, correct, rt, roundRT, trialPay, totalPay];
