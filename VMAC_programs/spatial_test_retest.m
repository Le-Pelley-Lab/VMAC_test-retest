...
elseif strcmp(experiment('session'), '2')  % second session
    oldData = load([datafilename, '1.mat'], 'DATA');  % load previous session's data
    starting_total = oldData.DATA.bonusSoFar;  % $$$
    points_starting = oldData.DATA.points_total;  % points
    clear oldData;
else
...
bonus_payment = ceil(bonus_payment/20) * 20;  % round up to nearest 20c
...
update_details(experiment, bonus_payment, 1);  % DATA is saved here
