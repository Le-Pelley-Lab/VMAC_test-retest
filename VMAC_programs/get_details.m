    ...
    while true
        ...
        if exist([data_filename, session, '.mat'], 'file') == 2
            ...
            if crash == 1
                ...
                % experiment data
                load([data_filename, session, '.mat'], 'experiment');

                disp(['Session:          ', experiment('session')])
                exptSession = str2num(experiment('session'));
                ...
                if ismember(confirm, accept_options)
                    experiment('postcrashstart') = {datestr(now, 0)};
                    break;
                else
                    clc;
                end
                ...
                clc;
                data_exists = 'Session %s data for participant %s already exists.\n\n';
            ...
            if str2double(session) == 1  % first session
                ...
                if testing == 0  % experimental version
                    ...
                    while true
                        ...
                        counterbalance = input('Counterbalance condition (1-4) ---> ');
                    ...
                    while true
                        ...
                        try
                            hand = input('Participant dominant hand (use first letter): left/right --> ', 's');  % stored as a string to stop Matlab accepting expressions or named variables as inputs
                        catch
                            % do nothing with errors, hand loop will repeat
                        end
...
%                     end
                    ...
                    if isKey(experiment, 'bonus_total')  % won't exist for experiments without a bonus
                        ...
                        experiment('bonus_session1') = experiment('bonus_total');
                    end
                    ...
                    if ismember(confirm, accept_options)
                        ...
                        if isKey(experiment, 'postcrashstart')
                            remove(experiment, 'postcrashstart');
                        end
